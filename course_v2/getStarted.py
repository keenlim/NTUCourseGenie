...
if 'last_updated' not in st.session_state:
    st.session_state.last_updated = {
        "degree": list(DEGREE_OPTIONS.values())[0], 
        "cohort": COHORT[0],
        "degree_key": list(DEGREE_OPTIONS.keys())[0], 
        "degree_type": DEGREE_TYPE[list(DEGREE_OPTIONS.keys())[0]][0], 
        "career": None, 
        "year_standing": list(YEAR_STANDING.values())[0], 
        "semester": SEMESTER[0]
    }
...
<div class="gradient-text">Get Started.</div>
"""

st.markdown(gradient_title_html, unsafe_allow_html=True)

st.write(f"To get started, we would like you to fill up some information.")

# st.write(st.session_state)

st.subheader("Academic Standing")
degree = st.selectbox("Degree *", 
                      options=DEGREE_OPTIONS.values(), 
                      index=list(DEGREE_OPTIONS.values()).index(st.session_state.last_updated['degree'])
                    )
...
with col1:
    cohort = st.selectbox("Cohort *", 
                          options = COHORT, 
                          index=COHORT.index(st.session_state.last_updated['cohort'])
                        )
with col2:
    degree_key = list(filter(lambda x: DEGREE_OPTIONS[x] == degree, DEGREE_OPTIONS))[0]
    degree_type = st.selectbox("Degree Type *", 
                               options = DEGREE_TYPE[degree_key], 
                               index=DEGREE_TYPE[degree_key].index(st.session_state.last_updated['degree_type']) if st.session_state.last_updated['degree_type'] in DEGREE_TYPE[degree_key] else 0)
with col3:
...
career = st.multiselect("Career Interest", options=CAREER_OPTIONS)
...
try: 
    ...
    submitted = st.button("Submit", use_container_width=True, icon=":material/send:")
except Exception as e:
    submitted = st.button("Submit", use_container_width=True, icon=":material/send:", disabled=True, help=f"🚨 Please fill in all the required fields to continue.")
